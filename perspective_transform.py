...
if __name__ == '__main__':
    ...
    image_path = r"image/original_color/black/24.bmp"
    # image_path = r"image/original_color/pers.bmp"
    ...
    left = dx
    right = 3840 - dx
    _height = (right - left) * (3/4)

    dy = 270 * ratio
    top = dy
    bottom = _height + top

    matrix = cv2.getPerspectiveTransform(np.float32(pts), np.float32([[left, top], [left, bottom], [right, bottom], [right, top]]))

    img_pers2 = cv2.warpPerspective(img_orig, matrix, (4000, 3000))

    print(left, top, right, bottom)

    transformer = PerspectiveTransformer(width, height, pts, dx=320)
    img_pers = transformer.transform(img_orig)
    h, w = img_pers.shape[:2]

    height, width = img_orig.shape[:2]

    cv2.namedWindow("img", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("img", int(width/4), int(height/4))
    cv2.imshow("img", img_orig)
    cv2.namedWindow("img_pers", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("img_pers", int(w/6), int(h/6))
    cv2.imshow("img_pers", img_pers2)
    cv2.imwrite("img_pers.png", img_pers2)
    cv2.waitKey(0)
